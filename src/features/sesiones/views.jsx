...
import { ESTADO_SESION, TIPO_SESION, MODALIDAD, METODO_PAGO } from '../../lib/constants.js'
...
function EstadoBadge({ estado }) {
  const e = ESTADO_SESION[estado] || { label: estado, badge: 'neutral' }
  return <Badge variant={e.badge}>{e.label}</Badge>
}
        ...
        <div className="grid grid-cols-7 gap-2">
          {weeks.flat().map((day) => {
            ...
            return (
              <div
                key={key}
                className={`flex min-h-[112px] flex-col rounded-card border p-2 transition-colors ${
                  inMonth ? 'border-white/70 bg-white/55' : 'border-transparent bg-white/20'
                }`}
              >
                <button
                  onClick={() => onCreateOn(key)}
                ...
                <div className="flex flex-col gap-1">
                  {items.slice(0, MAX).map((s) => (
                    ...
                    >
                      ...
                      <span className="truncate font-caption text-[10px] font-bold text-content-primary">
                        {formatTime(s.hora_inicio)} {s.patient?.nombre}
                      </span>
                    </button>
                  ))}
                  {items.length > MAX && (
                    <span className="px-1.5 font-caption text-[10px] text-content-muted">+{items.length - MAX} más</span>
                  )}
                </div>
              </div>
            )
...
export function ListView({ sessions, onEdit, onCancel }) {
  ...
  return (
    <div className="divide-y divide-stroke/50">
      {rows.map((s) => {
        const cancellable = s.estado !== 'cancelada' && s.estado !== 'completada'
        return (
          <div key={s.id} className="flex flex-wrap items-center gap-x-4 gap-y-2 px-2 py-3 transition-colors hover:bg-white/50">
            <div className="w-24 flex-shrink-0">
              <p className="font-heading text-sm font-bold text-content-primary">{capitalize(new Intl.DateTimeFormat('es-MX', { weekday: 'short', day: 'numeric' }).format(new Date(s.fecha)))}</p>
            ...
            <div className="w-24 text-right">
              ...
              <p className="font-caption text-xs text-content-muted">{s.pagado ? METODO_PAGO[s.metodo_pago] || 'Pagada' : 'Sin pagar'}</p>
            </div>
            ...
            <div className="flex items-center gap-1">
              ...
              {cancellable && (
                <button onClick={() => onCancel(s)} className="rounded-full px-3 py-1.5 font-heading text-xs font-bold text-content-muted transition-colors hover:bg-brand-pink/10 hover:text-rose-600">Cancelar</button>
              )}
            </div>
          </div>
        )
