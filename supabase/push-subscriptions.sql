...
-- RLS: each therapist manages only her own rows (my_terapeuta_id()); the owner
-- has full access; the notify Netlify function reads with the service role.

create table if not exists push_subscriptions (
  id uuid primary key default gen_random_uuid(),
  terapeuta_id uuid not null references therapists(id) on delete cascade,
